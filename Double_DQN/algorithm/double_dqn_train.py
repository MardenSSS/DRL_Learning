    ...
    for i in range(10):
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 10)):
                ...
                pbar.update(1)
    env.close()
    agent.save('../model/double_dqn_model.pth')
...
if __name__ == '__main__':
    ...
    agent = DQN(state_dim, HyperParams.hidden_dim, action_dim, HyperParams.lr, HyperParams.gamma, HyperParams.epsilon,
                HyperParams.target_update, HyperParams.device, 'DoubleDQN')
    ...
    plt.savefig('../results/DoubleDQN_moving_average_return.png')
    # 清除当前figure上的所有内容
    plt.clf()
    ...
    plt.ylabel('Q value')
    plt.title('DQN on {}'.format(HyperParams.env_name))
    plt.savefig('../results/DoubleDQN_max_q_value_list.png')
